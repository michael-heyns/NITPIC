...
int ShutDown = 0;                       	// =1 when no longer compiling
int DemoMACcounter;                     	// used during MAC include
int DemoSRCcounter;                     	// used during SRC compile
...
main(int argc, char *argv[])
{
	...
	// open key file
	strncpy(KeyFile, argv[0], NAMELEN-1);
	strcpy(&KeyFile[strlen(KeyFile)-4], ".KEY");
	read_key();
...
Add2FileList(char *Name)
{
	...
	{
		if ((Licensed > 0 && NextFile < MAXFILES - 1) ||
		    (!Licensed && NextFile < DEMOMAXFILES - 1))
		{
...
infile(char *Name)
{
	...
	if (FirstTime == Yes)
	{
		...
		MaxLinesCounter = 0;    // reset so DEMO user has his own lines
		TopLayer = Yes;
	}
	...
	{
		LINE();
		if (!ReadLicense && ShutDown)
			break;
	}

	if (!Licensed && ShutDown)
	{
		warning("Warning %s %d : DEMO COPY: Exceeding demo limits\n", INFILE, NR);
		exit(1);
	}
...
getline()
{
	...
	// test for DEMO shutdown
	if (!Licensed)
	{
		if (BusyWithMAC)
		{
			DemoMACcounter++;
			if (!ShutDown && DemoMACcounter > DEMOMACOVERHEAD)
				ShutDown = 1;
		}
		else
		{
			DemoSRCcounter++;
			if (!ShutDown && DemoSRCcounter > DEMOMAXSOURCE)
				ShutDown = 1;
		}
		if (!ShutDown && TotalNR > (DEMOMAXSOURCE + DEMOMACOVERHEAD))
			ShutDown = 1;
	}
	...
	while ((ptr = strchr(ptr, '\\')) && !isalpha(*(ptr+1)))
	{
		...
		*ptr = '\0';
		...
		if (!Licensed)
		{
			...
			{
				DemoMACcounter++;
				if (!ShutDown && DemoMACcounter > MACOVERHEAD)
					ShutDown = 1;
			}
			else
			{
				DemoSRCcounter++;
				if (!ShutDown && DemoSRCcounter > MAXSOURCE)
					ShutDown = 1;
			}
			if (!ShutDown && TotalNR > (MAXSOURCE + MACOVERHEAD))
				ShutDown = 1;
		}
	}
...
fracture()
{
	...
	if (!Licensed && !ShutDown && TotalNR > (MAXSOURCE + MACOVERHEAD))
		ShutDown = 1;;
